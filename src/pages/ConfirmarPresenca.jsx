...
import { FaUser, FaLock, FaCalendarAlt, FaUserShield, FaEye, FaEyeSlash, FaSignOutAlt, FaShare, FaMagic } from 'react-icons/fa';
...
export default function ConfirmarPresenca() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const nomeParaLogin = formData.nome;
      ...
      if (response.data.success) {
        // Salva o token persistente para futuros logins automáticos
        localStorage.setItem('persistentAuthToken', response.data.persistentToken);
        // Usa a sessão e os dados do jogador retornados para autenticar imediatamente
        setJogadorLogado(response.data.jogador);
        setSessionId(response.data.sessionId);
        setAutenticado(true);
          ...
          <AnimatePresence mode="wait">
            {modo === 'jogador' && (
              tentandoLoginAutomatico ? (
                <motion.div key="auto-login-spinner" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="text-center py-10 space-y-4">
                  <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-blue-500 mx-auto"></div>
                  <p className="text-blue-300 font-semibold flex items-center justify-center gap-2">
                    <FaMagic /> Verificando sua identidade...
                  </p>
                </motion.div>
              ) : !autenticado ? (
                <motion.form
